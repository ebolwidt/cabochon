  ...
  def write_extended(file)
    ...
    for i in 0 .. partitions.length - 1
      ...
      if (i + 1 < partitions.length)
        ...
        ebr[462, 16] = create_ebr_pointer(logical_lba_end + 1 + offset - lba_start, next_logical)
        offset = offset + (logical_lba_end + 1)
      else
        ebr[462, 16] = "\0" * 16
      end
      file.write_sector(ebr, write_offset)
    end
  end
