...
class TestGuidPartitionTable < Test::Unit::TestCase
  ...
  def test_known_layout
    gpt = nil
    File.open("testdata/gpt.img", "rb") do |file|
      gpt = GuidPartitionTable.read(file)
    end
    
    assert_equal("43B7D917-FBAB-45A5-BD3C-C68118C787A5", gpt.disk_guid.to_s.upcase)
    verify_partitions(gpt)
  end
  
  # TODO: Test backup table for known layout
  
  def test_write_read
    gpt = GuidPartitionTable.new_table
    gpt.disk_guid = UUIDTools::UUID.parse("43B7D917-FBAB-45A5-BD3C-C68118C787A5") 
    ...
    puts("Writing partition table #{gpt}")
    ...
    puts("Going to read back")
    
    File.open(path, "rb") do |file|
      gpt = GuidPartitionTable.read(file)
    end
    
    assert_equal(disk_guid.to_s.upcase, gpt.disk_guid.to_s.upcase)
    verify_partitions(gpt)
